...
const logger = helper.getLogger('upgrade-chaincode');

const upgradeChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
	...
	let success = true;
	let message = util.format('Successfully upgrade chaincode in organization %s to the channel \'%s\'', org_name, channelName);
	if (error_message) {
